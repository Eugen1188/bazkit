...
def estimate_open_price(barcode, quantity, unit, mode="purchase"):
    ...
    lookback_days = int(getattr(settings, "OPEN_PRICES_LOOKBACK_DAYS", 180))
    ...
    for item in items:
        ...
        if str(location.get("osm_address_country_code") or "").upper() != country_code:
            continue
    ...
    if not prices:
        return unavailable("Für dieses Produkt wurden keine aktuellen Preise in Deutschland gefunden.")
    ...
    return {
        ...
        "message": "Median der neuesten Preise je Geschäft.",
    }
